...
//
//        
//        boolean success = databaseHandler.addGuestfortable(firstName1, lastName1, phoneNumber, email, bookingType, checkinStatus,capacity,tableno,starttime, endtime, reservationDate);
//
//        if (success) {
...
//    }
    ...
    {
    	Parent root = FXMLLoader.load(getClass().getResource("/application/booktable.fxml"));
        Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
        Scene scene = new Scene(root);
        String css= this.getClass().getResource("/application/application.css").toExternalForm();
        scene.getStylesheets().add(css);
